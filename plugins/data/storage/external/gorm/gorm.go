...
func New(dsn string) (s *Storage, err error) {
	...
	case "sqlite3":
		...
		if s.dbh, err = gorm.Open("sqlite3", dsn); err != nil {
			return nil, err
		}
	case "mysql":
		if dsn == "" {
		...
		if s.dbh, err = gorm.Open("mysql", dsn); err != nil {
			return nil, err
		}
	case "postgres":
		if dsn == "" {
		...
		if s.dbh, err = gorm.Open("postgres", dsn); err != nil {
			return nil, err
		}
	default:
		return nil, errors.New("Unkown database type")
	}
...
func (s *Storage) Init(dsn string) error {
	...
	if s.dbh == nil {
		...
		case "sqlite3":
			...
			if db, err = gorm.Open("sqlite3", dsn); err != nil {
				return nil, err
			}
		case "mysql":
			if dsn == "" {
