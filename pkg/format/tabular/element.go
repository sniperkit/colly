...
import (
	tablib "github.com/sniperkit/colly/plugins/data/transform/tablib"
)
...
// TAB_FORMATS_IMPORT is the slice strings indexing all formats supported for tablib data import.
// TAB_FORMATS_IMPORT = []string{TAB_JSON, TAB_YAML, TAB_XML, TAB_CSV, TAB_TSV}

// TAB_FORMATS_EXPORT is the slice strings indexing all export formats supported for tablib data.
// TAB_FORMATS_EXPORT = []string{TAB_JSON, TAB_YAML, TAB_XML, TAB_CSV, TAB_TSV, TAB_XLSX, TAB_MD, TAB_MYSQL, TAB_POSTGRES}

//-- End
)

// Create aliases with struct defined in tablib pkg
type (

	// `TABDynamicColumn` is an alias of...
	TABDynamicColumn = tablib.DynamicColumn

	// `TABFormat` is an alias of...
	TABFormat = tablib.Format

	// `TABHook` is an alias of...
	TABHook = tablib.Hook

	// `TABOutput` is an alias of...
	TABOutput = tablib.Printer

	// `TABRanger` is an alias of...
	TABRanger = tablib.Ranger

	// `TABSlicer` is an alias of...
	TABSlicer = tablib.Slicer

	// `TABWriter` is an alias of...
	TABWriter = tablib.Writer

	// `TABDynamicColumn` is an alias of...
	// TABSelector = tablib.Selector
...
type TABElement struct {
	...
	// Dataset represents...
	Dataset *tablib.Dataset `json:"-" yaml:"-" toml:"-" xml:"-" ini:"-" csv:"-"`
...
// NewTABElementFromTABNode instanciates a new TABElement extracted with a tablib data query
func NewTABElementFromTABNode(resp *Response, query string, ds *tablib.Dataset) *TABElement {
	t := &TABElement{
		Dataset:  ds,
		Name:     query,
...
func NewTABElementFromTABSelect(resp *Response, hook *TABHook, ds *tablib.Dataset) *TABElement {
	t := &TABElement{
		Dataset:  ds,
		Hook:     hook,
		Request:  resp.Request,
